...
(function () {
  this.jKanban = function () {
    ...
    this.addElement = function (boardID, taskID, element, position) {
      ...
      nodeItem.setAttribute("id", element.taskID)
    ...
    function __buildItemCard(item) {
      ...
      var date = 'date' in item ? item.date : '';

      const calendarIcon = '<i class="far fa-clock" style="color:grey" aria-hidden="true"></i>'
      let content = '<div class="text-wrap">' + `<div class=" text-wrap"><p class="fs-6"> ${title}</p></div>` + `<div><p class="fw-light" style="color:grey; font-size: medium">${calendarIcon} ${date} </p></div>` + '</div>'
      ...
      if (self.options.itemHandleOptions.enabled) {
          if ((self.options.itemHandleOptions.customHandler || undefined) === undefined) {
              ...
              content = '<div class=\'item_handle ' + customCssHandler + '\'><i class=\'item_handle ' + customCssIconHandler + '\'></i></div' + content
          } else {
              content = '<div> ' + self.options.itemHandleOptions.customHandler.replace(/%([^%]+)%/g, function (match, key) 
...
$('#create_column_modal').click(function (event) 
{
...
$('#create_task_modal').click(function (event) 
{
  console.log(event.currentTarget)
  if($(event.target).closest("#modal_body").length === 0) {
